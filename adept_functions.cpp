...
//// THERE ARE TWO POSSIBLE FUNCTIONS AV_n_1, depending on whether we want to use data or not ////
...
/*adouble aV_n_1(int N, int nx, int ny, int jx, int jy, int np, double dt,
               const vector<double> & xi0, const vector<vector<double>> & y,
               const vector<double> & mu_thetas, const vector<double> & sigma_thetas, const vector<adouble> & x){
    ...
    //out += 0.5 * pow(xi0[nx]-x[np + nx*jx],2)/pow(sigma_xi,2) + (ny+1)*log(sigma_z) + (nx+1)*log(sigma_xi);
    //// **************************************************** ////
    //// **************************************************** ////
    ...
    for (size_t s = 1; s <= nx; ++s){
        ...
        //out += 0.5 * pow(xi0[s-1]-x[np + (s-1)*jx],2)/pow(sigma_xi,2) + (tau/(4.0*(double)jx*dt)) * pow(x[np + (s-1)*jx]-x[np + s*jx], 2);
        //// **************************************************** ////
        //// **************************************************** ////
            ...
            const vector<size_t> & non_zeros_event1, const vector<size_t> & zeros_event1)
{
    ...
    stack.new_recording();
    (aV_n_1(n_u, nx, ny, jx, jy, n_theta, dt, x0, y, mu_thetas, sigma_thetas, x, non_zeros_event1, zeros_event1)).set_gradient(1.0);
